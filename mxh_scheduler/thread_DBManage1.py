...
class DBManger:
    ...
    def get_aggregate1(self,time_day,time_now,time_now_n):
        ...
        res_Open = self.__col_KDatas.aggregate([
            ...
            {
                "$project": {"UpdateTime": 1, "ActionDay": 1, "LastPrice": 1, "_id": 0}
            },
            {
                "$sort": {"UpdateTime":1}
            },
...
if __name__== "__main__":
    ...
    k_list = n.split(",")
    thread_list = []
    loops = range(len(k_list))
    for i in loops:
        t = MyThread(db1.get_5_date,(start_datetime,end_datetime,k_list[i]),db1.get_aggregate1.__name__)
    ...
    #将数据写入csv文件
    for i in loops:
        res = thread_list[i].get_result()
        file_name = future_name +'_'+k_list[i]+'_'+start_datetime+"_"+end_datetime+".csv"
        dataframe = pandas.DataFrame(res)
        dataframe.to_csv(file_name,index=False,sep=',')

        # print("%s 分钟K线数据为：%s" %(k_list[i],res))
    # res = thread_list[0].get_result()
    # print('最终结果：',res)
    # print(len(res),res[0])
        ...
        # #包括开始日期，不含结束日期
        # res = db1.get_5_date(start_datetime,end_datetime,i)
        # for item in res:
        #     print(item)
    # print(res)
